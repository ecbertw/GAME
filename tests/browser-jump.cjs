...
(async()=>{
 ...
 try{
  async function pageFor(p,viewport={width:1200,height:1000}){
   ...
   const page=await ctx.newPage();page.on('pageerror',e=>{errors.push(e.message);console.log('PAGEERROR',e.message)});page.on('console',m=>{const t=m.text();if(t.startsWith('__'))console.log('PAGE',t)});

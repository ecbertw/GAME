(function(){
 ...
 let renderSerial=0;
 async function render(){const sequence=++renderSerial,isJump=!!window.eixoJumpActive;const modal=document.getElementById('rankingModal'),list=document.getElementById('fullRankingList');if(!modal||modal.classList.contains('hidden')||!list)return;const active=document.querySelector('.ranking-tab.active')?.id;const countryMode=active==='modalCountryTab';let page=1;const m=document.getElementById('pageInfo')?.textContent?.match(/^(\d+)/);if(m)page=Number(m[1])||1;let p=null;try{p=JSON.parse(localStorage.getItem('eixo_player')||'null')}catch(_){}const country=p?.country||'PT';try{const base=isJump?'/api/jump/rankings':'/api/rankings';const u=countryMode?`${base}?country=${encodeURIComponent(country)}&page=${page}`:`${base}?page=${page}`;const r=await fetch(u,{cache:'no-store'}),d=await r.json();if(!r.ok)throw Error();if(sequence!==renderSerial||isJump!==!!window.eixoJumpActive||modal.classList.contains('hidden'))return;document.getElementById('rankingModalTitle').textContent=isJump?'RANKING · JUMP':'RANKING';document.getElementById('pageInfo').textContent=page+' / '+d.pages;document.getElementById('prevPage').disabled=page<=1;document.getElementById('nextPage').disabled=page>=d.pages;list.innerHTML=d.players.length?d.players.map((x,i)=>{let tags='';if(x.worldRank<=3)tags+=tag('world',x.worldRank,'',x);if(x.countryRank<=3)tags+=tag('country',x.countryRank,String(x.country||'').toUpperCase(),x);const color=String(x.nameColor||'#fff').toLowerCase(),effect=x.nameEffect&&x.nameEffect!=='none'?` effect-${esc(x.nameEffect)}`:'',vip=Number(x.vipLevel||0);const hasLetters=vip>0&&Array.isArray(x.letterStyles)&&x.letterStyles.length,rainbow=color==='rainbow'&&!hasLetters,colorStyle=rainbow?'':' style="color:'+esc(color)+';"';return `<li><span class="full-rank-number">${(page-1)*25+i+1}</span><span class="full-player"><span class="rank-name-wrap"><span class="rank-player-name${rainbow?' name-rainbow':''}${effect}${hasLetters?' vip-letter-styled':''}"${colorStyle}>${hasLetters?letters(x.visualName||x.name,x.letterStyles):[...String(x.visualName||x.name)].map(ch=>`<span class="name-letter">${esc(ch)}</span>`).join('')}</span>${tags}${vipTag(vip)}</span></span><span class="full-score rank-score-wrap"><span class="rank-flag" title="${esc(x.country)}">${esc(flag(x.country))}</span><span class="rank-score">${Number(x.score||0)}</span></span></li>`}).join('') :'<li class="empty-full">'+(window.eixoT?window.eixoT('emptyFull','THERE ARE NO PLAYERS YET'):'THERE ARE NO PLAYERS YET')+'</li>';}catch(_){}}
 ...
 bindMainRanking();
 // JUMP uses the existing ranking modal and styles, but intercepts pagination
 // before legacy PULSE handlers can request PULSE scores.
 for(const id of ['modalWorldTab','modalCountryTab','prevPage','nextPage']){
  document.getElementById(id)?.addEventListener('click',e=>{
   if(!window.eixoJumpActive)return;
   e.preventDefault();e.stopImmediatePropagation();
   const world=document.getElementById('modalWorldTab'),country=document.getElementById('modalCountryTab');
   const info=document.getElementById('pageInfo');
   const match=String(info.textContent).match(/(\d+)\s*\/\s*(\d+)/);
   const current=Math.max(1,Number(match?.[1]||1)),total=Math.max(1,Number(match?.[2]||1));
   let next=current;
   if(id==='modalWorldTab'||id==='modalCountryTab'){
     const toWorld=id==='modalWorldTab';
     world.classList.toggle('active',toWorld);country.classList.toggle('active',!toWorld);next=1;
   }else if(id==='prevPage')next=Math.max(1,current-1);
   else next=Math.min(total,current+1);
   info.textContent=next+' / '+total;
   render();
  },true);
 }
 window.eixoRefreshFullRanking=render;

(function(){
 ...
 const tag=(t,n,country)=>`<span class="rank-tag ${t}-${n}">${n}# ${t==='country'?esc(country):'GLOBAL'}</span>`;const vipTag=n=>n>0?`<span class="vip-rank-tag vip-rank-${n}">VIP #${n}</span>`:'';const letters=(text,styles)=>{const arr=Array.isArray(styles)?styles:[];return [...String(text||'')].map((ch,i)=>{const s=arr[i]||{};const color=String(s.color||'').toLowerCase();const effect=String(s.effect||'none');const safeColor=/^#[0-9a-f]{6}$/i.test(color)?color:'';const safeEffect=/^[a-z]+$/.test(effect)?effect:'none';return `<span class="name-letter effect-${safeEffect}" style="${safeColor?'color:'+safeColor+';':''}">${esc(ch)}</span>`}).join('')};

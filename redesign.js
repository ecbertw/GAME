...
function labels(){
 ...
 views.passport.insertAdjacentHTML('afterbegin','<div class="rx-panel rx-progress"><div><p class="rx-eyebrow">EIXO LEVEL</p><strong id="rxLevel">—</strong></div><div class="rx-progress-track" aria-label="EXP"><i id="rxExpBar"></i></div><span id="rxExpText">— EXP</span></div>');
 views.passport.querySelectorAll('.rx-achievement').forEach((e,i)=>e.dataset.badge=['first-100','skybound','explorer'][i]);
 views.passport.querySelector('.rx-row .rx-tag').textContent=t('ATIVAS','ACTIVE');
 views.passport.querySelector('.rx-note').textContent=t('Ganha EXP em partidas validadas de JUMP e PULSE. As badges e os níveis desbloqueiam novas opções ao longo do teu percurso.','Earn XP in validated JUMP and PULSE runs. Badges and levels unlock new options throughout your journey.');
...
async function refreshProfile(){
 ...
 try{
  const [cosmetics,rank,progress,pulseRank]=await Promise.allSettled(['/api/jump/cosmetics','/api/jump/player-rank','/api/passport','/api/pulse/orbit/player-rank'].map(async path=>{const res=await fetch(path+'?id='+encodeURIComponent(p.id),{credentials:'same-origin',cache:'no-store'});if(!res.ok)throw Error('profile');return res.json();}));
  ...
  if(progress.status==='fulfilled'){const data=progress.value,total=Math.max(1,Number(data.nextLevelExp)||1),current=Math.max(0,Number(data.levelExp)||0);$('rxLevel').textContent=String(data.level);$('rxExpText').textContent=current+' / '+total+' EXP';$('rxExpBar').style.width=Math.min(100,current/total*100)+'%';const earned=new Set((data.badges||[]).map(x=>x.badge));views.passport.querySelectorAll('[data-badge]').forEach(card=>{const ok=earned.has(card.dataset.badge);card.classList.toggle('rx-locked',!ok);card.querySelector('small').textContent=ok?t('Conquistada','Earned'):t('Por conquistar','Locked');});}
  if(cosmetics.status==='rejected'||rank.status==='rejected'||progress.status==='rejected'||pulseRank.status==='rejected')$('rxProfileStatus').textContent=t('Alguns dados não estão disponíveis de momento.','Some profile data is temporarily unavailable.');

...
(function(){
 ...
 function note(freq,dur,wave,gain,dest,at){if(!ensure()||!freq)return;const t=ctx.currentTime+at,o=ctx.createOscillator(),g=ctx.createGain();o.type=wave;o.frequency.setValueAtTime(freq,t);g.gain.setValueAtTime(.0001,t);g.gain.exponentialRampToValueAtTime(gain,t+.01);g.gain.exponentialRampToValueAtTime(.0001,t+dur);o.connect(g);g.connect(dest);o.start(t);o.stop(t+dur+.03)}
 function noise(dur,gain,at,filterFreq){if(!ensure())return;const t=ctx.currentTime+at,b=ctx.createBuffer(1,Math.floor(ctx.sampleRate*dur),ctx.sampleRate),d=b.getChannelData(0);for(let i=0;i<d.length;i++)d[i]=(Math.random()*2-1)*(1-i/d.length);const s=ctx.createBufferSource(),g=ctx.createGain(),f=ctx.createBiquadFilter();s.buffer=b;f.type='highpass';f.frequency.value=filterFreq;g.gain.setValueAtTime(gain,t);g.gain.exponentialRampToValueAtTime(.0001,t+dur);s.connect(f);f.connect(g);g.connect(siteGain);s.start(t);s.stop(t+dur+.01)}
 function kick(at){note(120,.11,'sine',.055,siteGain,at)}
 function snare(at){noise(.075,.025,at,1800)}
 function hat(at){noise(.025,.007,at,6500)}

 const sections=[
  {bpm:82,mel:[392,392,440,523.25,587.33,523.25,440,392,329.63,392,440,523.25,440,392,329.63,0],bass:[196,196,220,220,261.63,261.63,220,220]},
  {bpm:82,mel:[523.25,523.25,587.33,659.25,783.99,659.25,587.33,523.25,440,493.88,523.25,587.33,659.25,587.33,523.25,0],bass:[261.63,261.63,293.66,293.66,329.63,329.63,293.66,293.66]},
  {bpm:84,mel:[329.63,329.63,392,440,493.88,523.25,493.88,440,392,440,523.25,587.33,659.25,587.33,523.25,440],bass:[164.81,164.81,196,196,220,220,246.94,246.94]},
  {bpm:80,mel:[392,440,523.25,587.33,659.25,587.33,523.25,440,392,329.63,392,440,523.25,659.25,587.33,392],bass:[196,196,220,220,261.63,261.63,220,220]}
 ];
 function playSection(si){
  ...
  s.mel.forEach((m,i)=>{
    const t=i*step;
    if(m)note(m,step*.88,'square',.032,siteGain,t);
    if(i%4===0)note(s.bass[(i/2)%s.bass.length|0],beat*1.7,'triangle',.014,siteGain,t);
  });
  return beat*8+350;
 }
 ...
 /* Bridge to the existing game: feedback text changes are the authoritative result signal.
    This makes effects work even if script load order changes. The fx gate prevents duplicates. */
 function bindGameFeedback(){
  const el=document.getElementById('feedback');if(!el||el.dataset.audioBridge)return;
  el.dataset.audioBridge='1';
  new MutationObserver(()=>{const v=(el.textContent||'').trim();if(v==='+2')perfect();else if(v==='+1')hit();else if(v==='MISS')miss()}).observe(el,{childList:true,characterData:true,subtree:true});
 }

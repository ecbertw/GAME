...
(function(){
 ...
 let ctx=null,master=null,siteGain=null,gameGain=null,started=false,timer=null,lastFx='',lastFxAt=0,jumpTimer=null,jumpBiomeName='',jumpStep=0;
 const clamp=v=>Math.max(0,Math.min(1,Number(v)||0));
 function save(){try{localStorage.setItem(KEY,JSON.stringify(state))}catch(_){}}
 function ensure(){if(ctx)return true;try{ctx=new(window.AudioContext||window.webkitAudioContext)();master=ctx.createGain();siteGain=ctx.createGain();gameGain=ctx.createGain();master.gain.value=.72;siteGain.gain.value=state.site;gameGain.gain.value=state.game;siteGain.connect(master);gameGain.connect(master);master.connect(ctx.destination);return true}catch(_){return false}}
 ...
 function jumpBiome(name){
  ...
  if(siteGain&&ctx)siteGain.gain.setTargetAtTime(state.site*.12,ctx.currentTime,.08);
  const tick=()=>{if(!ready())return;const i=jumpStep++,n=theme.notes[i%theme.notes.length],b=theme.bass[Math.floor(i/2)%theme.bass.length];note(n,.18,theme.wave,.011,gameGain,0);if(i%2===0)note(b,.35,'sine',.0075,gameGain,.015);if(biome==='city'&&i%4===3)note(n*2,.05,'square',.0055,gameGain,.08);if(biome==='forest'&&i%5===4)note(n*1.5,.28,'sine',.006,gameGain,.11);if(biome==='desert'&&i%4===2)note(n*.75,.24,'triangle',.006,gameGain,.1);if(biome==='snow'&&i%3===2)note(n*2,.4,'sine',.0045,gameGain,.14);};
 ...
 function controls(){
  ...
  sv.addEventListener('input',e=>{state.site=clamp(e.target.value);ensure();siteGain.gain.value=jumpBiomeName?state.site*.12:state.site;save();start()});
 ...
 bind();
 window.EixoAudio={hit,perfect,miss,pixel,pixelWind,jumpJump,jumpLand,jumpLose,jumpBiome,jumpStop,start,resume,setSiteVolume(v){state.site=clamp(v);ensure();siteGain.gain.value=jumpBiomeName?state.site*.12:state.site;save();start()},setGameVolume(v){state.game=clamp(v);ensure();gameGain.gain.value=state.game;save()}};
})();

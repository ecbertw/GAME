...
(function(){
 ...
 function bind(){controls();css();if(!document.body.dataset.audioBound){document.body.dataset.audioBound='1';const unlock=()=>resume().then(()=>start());document.addEventListener('pointerdown',unlock,{passive:true});document.addEventListener('keydown',unlock,{passive:true});document.addEventListener('touchstart',unlock,{passive:true})}}
 bind();
 window.EixoAudio={hit,perfect,miss,pixel,pixelWind,start,resume,setSiteVolume(v){state.site=clamp(v);ensure();siteGain.gain.value=state.site;save();start()},setGameVolume(v){state.game=clamp(v);ensure();gameGain.gain.value=state.game;save()}};
})();

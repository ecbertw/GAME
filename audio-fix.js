...
(function(){
 ...
 function playSection(si){if(!ensure())return;const s=sections[si%sections.length],beat=60/s.bpm,step=beat/2;for(let i=0;i<32;i++){const t=i*step,m=s.mel[i%16];if(m)note(m,step*.82,si%2?'square':'triangle',.025,siteGain,t);if(i%2===0)note(s.bass[(i/2)%8],beat*.82,'triangle',.021,siteGain,t);if(i%4===0)kick(t);if(i%4===2)snare(t);if(i%2===1)hat(t);if(i%8===0)s.chords[Math.floor(i/8)].forEach((n,j)=>note(n,.16,'square',.008,siteGain,t+j*.012))}const end=[523.25,587.33,659.25,783.99,659.25,523.25];end.forEach((n,i)=>note(n,.12,'square',.014,siteGain,beat*15.55+i*.08));return beat*16+180}

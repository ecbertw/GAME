/* EIXO ranking renderer: live Top 5, tags, flags, visual-name styling and personal rank. */
(function(){
 ...
 function setMyRank(id,rank){const el=document.getElementById(id);if(!el)return;const n=Number(rank);el.textContent=n>5?`${getRankLabel()} ${n}`:'';el.title=n>5?el.textContent:'';el.hidden=!(n>5);}
 ...
 async function load(){
   ...
   try{
     ...
     if(p?.id&&p?.token)requests.push(fetch('/api/player-rank?id='+encodeURIComponent(p.id)+'&token='+encodeURIComponent(p.token),{cache:'no-store'}));
     ...
     if(results[2]){
       ...
       else{setMyRank('worldMyRank',9999);setMyRank('nationalMyRank',9999);}
     }else{setMyRank('worldMyRank',9999);setMyRank('nationalMyRank',9999);}
   }catch(_){}
 }

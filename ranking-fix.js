...
(function(){
 ...
 function render(list,target){if(!list?.length){target.innerHTML='<li class="empty-row">AINDA SEM JOGADORES</li>';return}target.innerHTML=list.slice(0,5).map((p,i)=>{let tags='';if(Number(p.worldRank)<=3)tags+=tag('world',Number(p.worldRank));if(Number(p.countryRank)<=3)tags+=tag('country',Number(p.countryRank));const color=String(p.nameColor||'#fff').toLowerCase(),rainbow=color==='rainbow';return `<li><span class="rank-number">${i+1}</span><span class="rank-name-wrap">${tags}<span class="rank-player-name${rainbow?' name-rainbow':''}" style="${rainbow?'':'color:'+esc(color)+';'}">${esc(p.visualName||p.name)}</span></span><span class="rank-score-wrap"><span class="rank-flag" title="${esc(p.country)}">${flag(p.country)}</span><span class="rank-score">${Number(p.score||0)}</span></span></li>`}).join('')}

/* EIXO RGB color: a continuous RGB spectrum travels through the letters without a DOM mutation loop. */
(function(){
 ...
 function fixNames(){
  document.querySelectorAll('.rank-player-name.name-rainbow,.name-preview.name-rainbow').forEach(el=>{el.classList.remove('name-rainbow');el.classList.add('name-rgb');if(el.dataset.rgbReady==='1')return;el.style.color='transparent';el.style.removeProperty('background');el.style.removeProperty('-webkit-text-fill-color');el.querySelectorAll('.name-letter').forEach((letter,i)=>letter.style.setProperty('--i',i));el.dataset.rgbReady='1';});
  document.querySelectorAll('.rank-player-name.name-rgb,.name-preview.name-rgb').forEach(el=>{if(el.dataset.rgbReady==='1')return;el.querySelectorAll('.name-letter').forEach((letter,i)=>letter.style.setProperty('--i',i));el.dataset.rgbReady='1';});
 }

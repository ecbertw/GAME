(() => {
  ...
  function render(){
    ...
    playerName.textContent=p?(p.name+(p.role==='admin'?' [ADMIN]':p.role==='moderator'?' [MOD]':'')):'ENTRAR';
    const t=labels[lang()]||labels.en;
    const settings=lang()==='pt'?'DEFINIÇÕES':'SETTINGS',logout=lang()==='pt'?'SAIR DA CONTA':'LOG OUT';
    playerMenu.innerHTML=`${p?'<button type="button" class="player-option account-menu-option" data-action="settings">'+settings+'</button>':''}<button type="button" class="player-option" data-action="customize">${t.customize}</button><button type="button" class="player-option" data-action="bugs">${t.bugs}</button><button type="button" class="player-option" data-action="suggestions">SUGESTÕES</button><button type="button" class="player-option" data-action="contact">${lang()==='pt'?'CONTACTOS':t.contact}</button>${p?'<button type="button" class="player-option account-menu-option" data-action="logout">'+logout+'</button>':''}`;
  }
  ...
  render();window.addEventListener('eixo-auth-ready',render);window.addEventListener('storage',render);window.addEventListener('eixo-player-updated',render);
})();

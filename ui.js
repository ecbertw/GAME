(() => {
  ...
  function render(){let p=null;try{p=JSON.parse(localStorage.getItem('eixo_player')||'null')}catch(_){}playerName.textContent=p?.name||'JOGADOR';const t=labels[lang()]||labels.en;playerMenu.innerHTML=`<button type="button" class="player-option" data-action="customize">${t.customize}</button><button type="button" class="player-option vip-menu-toggle" data-action="vip">◆ ${t.vip}<span class="menu-arrow">▶</span></button><div class="vip-submenu"><button type="button" class="player-option vip-sub-option" data-action="vip-buy">COMPRAR VIP</button><button type="button" class="player-option vip-sub-option" data-action="vip-customize">PERSONALIZAR NOME VIP</button></div><button type="button" class="player-option" data-action="bugs">${t.bugs}</button><button type="button" class="player-option" data-action="suggestions">SUGESTÕES</button><button type="button" class="player-option" data-action="contact">${t.contact}</button>`;}
  ...
  if(action==='vip'){playerMenu.classList.toggle('vip-open');return}

...
- Existem quatro biomas: CITY, FOREST, DESERT e SNOW. **Não existe limite fixo de instâncias por bioma nem globalmente**: cada ambiente cria automaticamente tantas instâncias independentes de 5 jogadores quantas forem necessárias, conforme a procura. Não limitar o jogo inteiro a 20 jogadores.
- Ao clicar em JOIN SERVER, o jogador escolhe o bioma. O servidor procura uma instância desse mesmo bioma com vaga ou cria outra automaticamente, sem obrigar ninguém a mudar de ambiente. Uma instância cheia deve mostrar 5/5 e não permitir entradas adicionais. Exemplo: FOREST #1 (5/5), FOREST #2 (5/5), FOREST #3 (2/5); a próxima entrada vai para FOREST #3, nunca para outro bioma por falta de vagas.
...
- Não enviar atualizações de jogadores de biomas/instâncias diferentes. Não escrever a posição de cada frame na base de dados. Utilizar frequência de rede moderada (a calibrar com testes reais), interpolação de avatares remotos e limitar payloads. O número de instâncias não tem teto arbitrário, mas a capacidade real da VPS é finita: acompanhar consumo de CPU/RAM/latência e prever expansão para mais servidores caso a procura aumente, em vez de concentrar demasiados jogadores numa instância.

...
echo "[3/5] Syntax"
for f in server.js auth-server.js server-start.js paypal-server.js paypal-checkout.js ui.js vip-fix.js jump-server.js jump-team-server.js jump-physics.js jump-worlds.js jump.js; do node --check "$APP/$f"; done
...
for p in server.js auth-server.js server-start.js paypal-server.js jump-server.js jump-team-server.js package.json .git/HEAD .env; do

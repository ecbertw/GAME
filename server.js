...
async function handleApi(req,res,url){
 try{
  ...
  if(req.method==='GET'&&url.pathname==='/api/paypal/orders/status'){const p=await roomAuth(url.searchParams.get('id'),url.searchParams.get('token'));const orderId=url.searchParams.get('orderId');if(!boundedRate(paypalRate,'status:'+p.id+':'+orderId,180,10*60*1000))throw Object.assign(new Error('Demasiadas verificações de pagamento. Aguarda um momento.'),{status:429});return json(res,200,await paypalService.orderStatus(global.db,p,orderId));}

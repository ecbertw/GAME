...
(function(){
 ...
 $('leaveActiveRoom')?.addEventListener('click',()=>{$('roomBoard').classList.add('hidden');window.eixoActiveRoomId=null;if(typeof window.eixoRefreshRankings==='function')window.eixoRefreshRankings();});
  const abandonModal=$('abandonRoomModal');
  const closeAbandon=()=>abandonModal?.classList.add('hidden');
  $('abandonRoomClose')?.addEventListener('click',closeAbandon);
  $('abandonRoomCancel')?.addEventListener('click',closeAbandon);
  abandonModal?.addEventListener('click',e=>{if(e.target===abandonModal)closeAbandon()});
  $('abandonActiveRoom')?.addEventListener('click',()=>abandonModal?.classList.remove('hidden'));
  $('abandonRoomConfirm')?.addEventListener('click',async()=>{
    const a=getAuth(),id=window.eixoActiveRoomId;if(!a||!id)return;
    const b=$('abandonRoomConfirm');b.disabled=true;

...
(function(){
  ...
  function updateCountryPicker(){
    ...
    button.innerHTML='<img class="auth-country-flag" src="'+flagUrl(code)+'" alt=""><span class="auth-country-name">'+countryName(code).toUpperCase()+'</span><span class="auth-country-chevron">▼</span>';
    menu.innerHTML=codes.map(c=>'<button type="button" class="auth-country-option" role="option" aria-selected="'+String(c===code)+'" data-country="'+c+'"><img class="auth-country-flag" src="'+flagUrl(c)+'" alt=""><span class="auth-country-name">'+countryName(c).toUpperCase()+'</span><span class="auth-country-code">'+c+'</span></button>').join('');
  }
